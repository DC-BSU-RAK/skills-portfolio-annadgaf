...
class MathsQuiz:
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("Cute Math Quiz")
        ...
        self.bold_font = ("Arial", 12, "bold")  # Added bold font
    ...
    def setup_widgets(self):
        # Main frame
        self.main_frame = tk.Frame(self.window, bg='#ffe6f2')
        self.main_frame.pack(expand=True, fill='both', padx=20, pady=20)
        
        # Title label
        self.title_label = tk.Label(self.main_frame, text="⊹ ࣪ ˖ Math Quiz ⊹ ࣪ ˖", 
                                   font=self.title_font, bg='#ffb6e1', fg='#8b008b',
                                   padx=20, pady=10)
        self.title_label.pack(fill='x', pady=(0,20))
        
        # Question label
        self.question_label = tk.Label(self.main_frame, text="", 
                                      font=self.normal_font, bg='#ffe6f2', fg='#c71585')
        self.question_label.pack(pady=10)
        
        # Answer entry
        self.answer_entry = tk.Entry(self.main_frame, font=self.normal_font, 
                                    bg='#fff0f5', fg='#8b008b', width=20)
        self.answer_entry.pack(pady=10)
        
        # Submit button
        self.submit_btn = tk.Button(self.main_frame, text="Submit Answer", 
                                   font=self.button_font, bg='#ff69b4', fg='white',
                                   activebackground='#ff1493', command=self.check_answer)
        ...
        self.diff_frame = tk.Frame(self.main_frame, bg='#ffe6f2')
    ...
    def displayMenu(self):
        ...
        # Difficulty buttons
        easy_btn = tk.Button(self.diff_frame, text="₊˚⊹ 1. Easy", 
                            font=self.button_font, bg='#ffb6c1', fg='#8b008b',
                            command=lambda: self.set_difficulty("easy"))
        easy_btn.pack(pady=5, fill='x')
        
        moderate_btn = tk.Button(self.diff_frame, text="⋆˚࿔ 2. Moderate", 
                               font=self.button_font, bg='#ff91a4', fg='#8b008b',
                               command=lambda: self.set_difficulty("moderate"))
        moderate_btn.pack(pady=5, fill='x')
        
        advanced_btn = tk.Button(self.diff_frame, text="ᰔ 3. Advanced", 
                               font=self.button_font, bg='#ff6b95', fg='#8b008b',
    ...
    def displayProblem(self):
        ...
        # Create the question text with "Question X:" in bold and the math problem in normal
        question_text = f"Question {self.current_question + 1}: {self.num1} {self.operation} {self.num2} = ?"
        self.question_label.config(text=question_text, font=self.bold_font)  # CHANGED: Use bold font for the entire label
